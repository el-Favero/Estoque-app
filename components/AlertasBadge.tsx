...
const { 
  ...
  if (totalAlertas === 0 && produtosVencidos.length === 0) return null;
  ...
  return (
    <>
      ...
      >
        <Text style={styles.badgeText}>⚠️ {totalAlertas + produtosVencidos.length}</Text>
      </TouchableOpacity>
          ...
          <View style={styles.modalContent}>
            ...
            <View style={styles.tabsContainer}>
              ...
              >
                <Text style={[styles.tabText, abaAtiva === 'validade' && styles.tabTextAtiva]}>
                  Validade ({todosAlertas.filter(a => a.tipo === 'validade').length + produtosVencidos.length})
                </Text>
              </TouchableOpacity>
            </View>

            <FlatList
              data={abaAtiva === 'validade' ? [...filtrarAlertas(), ...produtosVencidos.map(p => ({
                produto: p,
                tipo: 'validade',
                mensagem: `${p.nome} VENCIDO há ${(p as any).diasAtraso} dias!`,
                gravidade: 'alta'
              }))] : filtrarAlertas()}
            ...
            <View style={styles.resumoContainer}>
              <View style={styles.resumoItem}>
                <Text style={styles.resumoValor}>{totalEstoqueMinimo}</Text>
